...
# from archs import Train

import archs
import losses
from dataset import Dataset
from loss import loss, dice
...
指定参数：
--dataset dsb2018_96 
--arch NestedUNet DeepLabV3Plus FPN
...
def parse_args():
    ...
    # model2222222
    parser.add_argument('--arch', '-a', metavar='ARCH', default='Train',
                        choices=ARCH_NAMES,
...
def main():
    ...
    elif config['arch'] == 'DoubleUnet' or 'DoubleAttentionUnet' or "Unet2D" or "AttionUnet":
        model = archs.__dict__[config['arch']]()
    elif config['arch'] == 'ResUNet18':
        model = archs.__dict__[config['arch']](archs.BasicBlock, [2, 2, 2, 2], num_classes=1, input_channels=3)
    elif config['arch'] == 'AFFDoubleUnet':
        model = archs.__dict__[config['arch']]()
    elif config['arch'] == 'oneAFFUNet':
        model = archs.__dict__[config['arch']]()
    else:
    ...
    # pretrained_weights = torch.hub.load_state_dict_from_url(model_url)
    #
    model = model.cuda()

    # new_model = Train()

    # # Step 2: Load the pre-trained model's state_dict.
    # pretrained_dict = torch.load('convnext_tiny_1k_224_ema.pth')
    #
    # # Step 3: Get the state_dict from the new model.
    # new_model_dict = new_model.state_dict()
    #
    # # Step 4: Filter out unnecessary keys from pretrained_dict.
    # pretrained_dict = {k: v for k, v in pretrained_dict.items() if k in new_model_dict}
    #
    # # Step 5: Update the new model's state_dict.
    # new_model_dict.update(pretrained_dict)
    #
    # # Step 6: Load the updated state_dict into new_model.
    ...
    for epoch in range(config['epochs']):
        ...
        val_dice.append(val_log['dice'])

        with open("models/dsb2018_96_Train_woDS/train_loss.txt", 'w') as train_los:
            train_los.write(str(train_loss))
        with open("models/dsb2018_96_Train_woDS/val_loss.txt", 'w') as val_los:
            val_los.write(str(val_loss))

        with open("models/dsb2018_96_Train_woDS/train_dice.txt", 'w') as train_dic:
